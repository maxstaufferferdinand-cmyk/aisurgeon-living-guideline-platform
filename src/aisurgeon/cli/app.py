...
@app.command("repair-and-rebuild-guideline-references-v2")
def repair_and_rebuild_guideline_references_v2_command(
    pdf: Annotated[Path, typer.Option("--pdf")],
    extraction_run: Annotated[Path, typer.Option("--extraction-run")],
    synthesis_run: Annotated[Path, typer.Option("--synthesis-run")],
    failed_reference_run: Annotated[Path, typer.Option("--failed-reference-run")],
    ...
    env_file: EnvFileOption = None,
    resume_run: Annotated[Path | None, typer.Option("--resume-run")] = None,
    pages_per_job: Annotated[int, typer.Option("--pages-per-job", min=1)] = 1,
    context_pages: Annotated[int, typer.Option("--context-pages", min=0)] = 1,
) -> None:
    """Repair bibliography with physical page slices, then rebuild the DOCX."""
    settings = _load_settings(env_file)
    if settings.gemini_api_key is None:
        typer.echo("GEMINI_API_KEY fehlt.", err=True)
        raise typer.Exit(2)
    try:
        run_dir = run_reference_repair_v2_and_rebuild(
            pdf=pdf,
            extraction_run=extraction_run,
            synthesis_run=synthesis_run,
            failed_reference_run=failed_reference_run,
        ...
        typer.echo(f"Referenz-Repair-v2/Rebuild fehlgeschlagen: {exc}", err=True)
        raise typer.Exit(4) from exc
    typer.echo(f"Reference-Repair-v2-Run-Verzeichnis: {run_dir}")
